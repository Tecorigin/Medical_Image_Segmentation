...
def verify_dataset_integrity(folder):
    ...
    for c in expected_train_identifiers:
        ...
        for i in range(num_modalities):
            expected_file = join(folder, "imagesTr", c + "_%04.0d.nii.gz" % i)
            if not isfile(expected_file):
                no_suffix_file = join(folder, "imagesTr", c + ".nii.gz")
                if isfile(no_suffix_file):
                    expected_image_files.append(no_suffix_file)
                else:
                    expected_image_files.append(expected_file)
            else:
                expected_image_files.append(expected_file)
    ...
    if len(expected_test_identifiers) > 0:
        ...
        for c in expected_test_identifiers:
            ...
            expected_image_files = []
            for i in range(num_modalities):
                expected_file = join(folder, "imagesTs", c + "_%04.0d.nii.gz" % i)
                if not isfile(expected_file):
                    no_suffix_file = join(folder, "imagesTs", c + ".nii.gz")
                    if isfile(no_suffix_file):
                        expected_image_files.append(no_suffix_file)
                    else:
                        expected_image_files.append(expected_file)
            ...
            assert all([isfile(i) for i in expected_image_files]), "some image files are missing for case %s. Expected files:\n %s\nActual files in directory:\n %s" % (
                c, expected_image_files, subfiles(join(folder, "imagesTs"), join=False, suffix=".nii.gz"))
