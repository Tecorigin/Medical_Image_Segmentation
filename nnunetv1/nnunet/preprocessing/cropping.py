...
def get_case_identifier(case):
    ...
    if filename.endswith(".nii.gz"):
        # 如果是 _0000.nii.gz 格式，去掉 _0000
        if filename.endswith("_0000.nii.gz"):
            case_identifier = filename[:-len("_0000.nii.gz")]
        else:
            # 原始格式 hippocampus_023.nii.gz
            case_identifier = filename[:-len(".nii.gz")]
    else:
        case_identifier = filename.split(".nii.gz")[0][:-5]  # 保持原有逻辑作为fallback
...
def find_actual_data_file(expected_file):
    """查找实际存在的数据文件"""
    if os.path.exists(expected_file):
        return expected_file
    
    # 如果标准格式不存在，尝试原始格式
    if expected_file.endswith('_0000.nii.gz'):
        original_file = expected_file.replace('_0000.nii.gz', '.nii.gz')
        if os.path.exists(original_file):
            return original_file
    
    return expected_file  # 返回原路径用于错误提示
...
def load_case_from_list_of_files(data_files, seg_file=None):
    ...
    # 修改：查找实际存在的文件
    actual_data_files = [find_actual_data_file(f) for f in data_files]
    
    # 检查文件是否存在
    for f in actual_data_files:
        if not os.path.exists(f):
            raise RuntimeError(f"File does not exist: {f}. Expected files: {data_files}")
    
    try:
        data_itk = [sitk.ReadImage(f) for f in actual_data_files]
    except Exception as e:
        print(f"Error reading files: {actual_data_files}")
        raise e

    properties["original_size_of_raw_data"] = np.array(data_itk[0].GetSize())[[2, 1, 0]]
    properties["original_spacing"] = np.array(data_itk[0].GetSpacing())[[2, 1, 0]]
    properties["list_of_data_files"] = data_files  # 保持原始文件列表
    properties["actual_data_files"] = actual_data_files  # 添加实际使用的文件列表
...
class ImageCropper(object):
    ...
    def load_crop_save(self, case, case_identifier, overwrite_existing=False):
        try:
            ...
            print(f"Expected data files: {case[:-1]}")
            if case[-1] is not None:
                print(f"Segmentation file: {case[-1]}")
            raise e
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        for j, case in enumerate(list_of_files):
            if case[-1] is not None:
                # 修改：复制前检查文件是否存在
                actual_seg_file = find_actual_data_file(case[-1])
                if os.path.exists(actual_seg_file):
                    shutil.copy(actual_seg_file, output_folder_gt)
                else:
                    print(f"Warning: Segmentation file not found: {case[-1]}")
