...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="ACDC_data", help="train data path")
...
def train_one_epoch(args, model, optimizer, scaler, train_loader, epoch, criterion, logger):
    if IS_MASTER:
    ...
    for batch, batched_input in enumerate(train_loader):
        ...
        if args.use_amp:
            with torch.cuda.amp.autocast():
                ...
                loss = criterion(masks.float(), labels.float(), iou_predictions.float())
        ...
        # 仅主进程打印
        if IS_MASTER and int(batch+1) % 50 == 0:
        ...
        for iter in range(args.iter_point):
            ...
            if IS_MASTER:
                print(f"\nEpoch: {epoch+1}, Batch: {batch+1}, Iter: {iter+1}/{args.iter_point}, loss: {loss.item():.4f}")
        ...
        if IS_MASTER and int(batch+1) % 200 == 0:
...
def main(args):
    ...
    scheduler = None
    if args.lr_scheduler and IS_MASTER:
    ...
    if IS_MASTER:
        if args.use_amp:
            print("*******Mixed precision with torch.amp")
        else:
    ...
    logger = get_logger(os.path.join(args.work_dir, "logs", f"{args.run_name}_{datetime.datetime.now().strftime('%Y%m%d-%H%M.log')}")) if IS_MASTER else None
    ...
    for epoch in range(0, args.epochs):
        if train_sampler is not None:
            train_sampler.set_epoch(epoch)  # 确保多卡shuffle一致
        model.train()
        train_metrics = {}
        start = time.time()
        if IS_MASTER:
            os.makedirs(os.path.join(f"{args.work_dir}/models", args.run_name), exist_ok=True)
        train_losses, train_iter_metrics = train_one_epoch(args, model, optimizer, scaler, train_loader, epoch, criterion, logger)

        if args.lr_scheduler is not None and IS_MASTER:
            scheduler.step()
        ...
        if IS_MASTER:
            ...
            logger.info(f"epoch: {epoch + 1}, lr: {lr}, Train loss: {average_loss:.4f}, metrics: {train_metrics}")
            ...
            if average_loss < best_loss:
                ...
                torch.save(state, save_path)
                print("保存最优模型:")
                best_save_path = os.path.join(args.work_dir, "models", args.run_name, "model_best.pth")
