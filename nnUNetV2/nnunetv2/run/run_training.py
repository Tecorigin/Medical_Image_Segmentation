...
def set_seed(seed: int):
    ...
    if torch.backends.flags_frozen():
        warnings.warn("PyTorch global flags support of backends is disabkled, enable it to set global `cudnn` flags.")
...
def setup_ddp(rank, world_size,device_type):
    ...
    if device_type == 'cuda':
        ...
        print(f"device: {device_type} init ddp error")
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size,device_type):
    setup_ddp(rank, world_size,device_type)
    #torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,device=torch.device(device_type))
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        assert device.type in ['cuda', 'sdaa'], f"DDP training (triggered by num_gpus > 1) is only implemented for cuda devices. Your device: {device}"
...
def run_training_entry():
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                    help="Use this to set the device the training should run with. Available options are 'cuda' "
                         "(GPU), 'cpu' (CPU) and 'mps' (Apple M1/M2) and 'sdaa' . Do NOT use this to set which GPU ID! "
                         "Use CUDA_VISIBLE_DEVICES=X nnUNetv2_train [...] instead!")
    args = parser.parse_args()
    set_seed(42)
    assert args.device in ['cpu', 'cuda', 'mps', 'sdaa'], f'-device must be either cpu, mps or cuda. Other devices are not tested/supported. Got: {args.device}.'
    if args.device == 'cpu':
