...
try:
    print("use sdaa")
    from torch.sdaa import device_count
except:
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        if self.is_ddp:  # implicitly it's clear that we use cuda in this case
            ...
            if self.device.type == 'sdaa':
                self.device = torch.device(type='sdaa', index=self.local_rank)
            if self.device.type == 'cuda':
                self.device = torch.device(type='cuda', index=self.local_rank)
        else:
            if self.device.type in ['cuda', 'sdaa']:
                # we might want to let the user pick this but for now please pick the correct GPU with CUDA_VISIBLE_DEVICES=X
                self.device = torch.device(type=self.device.type, index=0)
        ...
        try:
            if self.device.type == 'sdaa':
                print("self.grad_scaler = torch.sdaa.amp.GradScaler()")
                self.grad_scaler = GradScaler("sdaa")
        except:
            pass
    ...
    def get_dataloaders(self):
        ...
        if allowed_num_processes == 0:
            ...
            mt_gen_train = NonDetMultiThreadedAugmenter(data_loader=dl_tr, transform=None,
                                                        ...
                                                        pin_memory=self.device.type in ['cuda', 'sdaa'], wait_time=0.002)
            mt_gen_val = NonDetMultiThreadedAugmenter(data_loader=dl_val,
                                                      transform=None, num_processes=max(1, allowed_num_processes // 2),
                                                      num_cached=max(3, allowed_num_processes // 4), seeds=None,
                                                      pin_memory=self.device.type in ['cuda', 'sdaa'],
                                                      wait_time=0.002)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True)  if self.device.type in ['cuda', 'sdaa']  else dummy_context():
    ...
    def start_profiler(self):
        if os.getenv('TORCH_PROFILER_ENABLE') is not None:
            save_dir = './tmp'
            ...
            return torch.profiler.profile(
                ...
                record_shapes=True,
                with_stack=True,  # 启用调用栈信息
                with_modules=True,
                on_trace_ready=torch.profiler.tensorboard_trace_handler(save_dir)  # 保存路径
            )
        return None
    ...
    def run_training(self):
        self.on_train_start()
        # 记录当前进程的rank（DDP中使用，单卡为0）
        current_rank = self.local_rank  # 从类属性获取rank
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            self.on_train_epoch_start()
            train_outputs = []
            # 训练步骤：记录step并打印rank、step、loss
            for batch_id in range(self.num_iterations_per_epoch):
                # 计算当前全局step（epoch * 每轮迭代数 + batch_id）
                current_step = batch_id + 1
                global_step = epoch * self.num_iterations_per_epoch + batch_id
                # 执行训练步骤并获取loss
                step_output = self.train_step(next(self.dataloader_train))
                train_outputs.append(step_output)
                # 打印rank、step、loss（保留4位小数）
                print(f"Rank: {current_rank} | Epoch: {epoch} | Train Step: {batch_id}/{self.num_iterations_per_epoch} | Loss: {step_output['loss'].item():.4f}")
        #        if current_step == 10:
        #           print(f"Rank: {current_rank} | 达到step == ，退出程序")
        #            sys.exit(0)  # 退出程序
                if profiler is not None:
                    profiler.step()
            ...
            with torch.no_grad():
                self.on_validation_epoch_start()
                val_outputs = []
                # 验证步骤：记录step并打印rank、step、loss
                for batch_id in range(self.num_val_iterations_per_epoch):
                    # 计算当前验证全局step
                    global_val_step = epoch * self.num_val_iterations_per_epoch + batch_id
                    # 执行验证步骤并获取loss
                    step_output = self.validation_step(next(self.dataloader_val))
                    val_outputs.append(step_output)
                    # 打印rank、step、loss（保留4位小数）
                    print(f"Rank: {current_rank} | Epoch: {epoch} | Val Step: {batch_id}/{self.num_val_iterations_per_epoch} | Loss: {step_output['loss'].item():.4f}")
